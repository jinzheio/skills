---
name: jz-setup-personal-context
...
```bash
node <skill-dir>/scripts/install-profile.mjs -g
```
...
```bash
node <skill-dir>/scripts/install-profile.mjs --targets codex,claude-code
```

dry run：

```bash
node <skill-dir>/scripts/install-profile.mjs -g --dry-run
```
