...
三个通道按优先级使用：本地素材库（最快、质量可控）→ 免版权素材站（全自动但少电影感）→ AI 推荐片单（最贴合但需用户手动下载）。每个 B-roll 需求先过通道 1，未命中再走 2 或 3；必须要电影质感的段落（金句、强情绪）直接走通道 3。

## 通道 1：本地素材库

路径来自 `~/.config/skills/jz-video-package/config.yml` 的 `material_library`。推荐目录约定：
