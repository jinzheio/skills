...
将参考视频转化为当前项目的同风格宣传片。流程分为六个阶段：准备参考视频、分析参考视频、编写内容脚本、用 Remotion 构建、添加配乐、渲染验证。每个阶段都有对应的参考文档提供具体方案。按顺序执行各阶段；最常见的失败模式是在分析结果写下来之前就跳到代码阶段。
...
- 如果用户没有提供新参考视频，先列出当前项目 `references/video-style-clone/` 中已有的缓存参考视频，让用户选择。只列缓存条目的名称、来源文件名、时长和上次分析时间，不要重新分析。
- 如果 `references/video-style-clone/` 不存在，或里面没有可用的缓存参考视频，停止执行并告诉用户需要提供至少一个参考视频。
- 如果用户提供的视频已经有缓存，优先复用缓存的风格表、抽帧和音频分析；只有原文件变化、缓存缺失，或用户要求提高抽帧密度时才重新分析。

### 缓存目录

每次执行后，把参考视频原文件和分析中间结果放入当前项目的 `references/video-style-clone/<reference-id>/`：
...
- [ ] 所有参考视频原文件、抽帧、音频分析和风格表已写入 `references/video-style-clone/`
