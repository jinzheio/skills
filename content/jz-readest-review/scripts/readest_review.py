...
def normalize_base_url(config: dict[str, str]) -> str:
    ...
    if not base_url:
        raise SystemExit("Missing READEST_BASE_URL or READEST_PRIMARY_DOMAIN in ~/.config/skills/jz-readest-review/.env")
...
def create_session() -> Session:
    ...
    if missing:
        raise SystemExit(f"Missing {', '.join(missing)} in ~/.config/skills/jz-readest-review/.env")
