---
name: jz-get-douyin-transcripts
description: 将抖音视频转写为 Markdown。支持单个视频 URL 或抖音用户/主页批量转写。转录文件写入 raw/clippings/douyin 下对应的 channel 目录，通过 sync-state.json 记录进度以避免重复下载。默认获取最近 30 个视频，跳过超过 5 分钟的视频。
---
...
- 已为 `jz-transcribe-media` 配置好 GLM API key
