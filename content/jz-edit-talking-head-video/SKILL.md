---
name: jz-edit-talking-head-video
...
3. 剪映后端时：草稿文件夹路径？（默认：读 `~/.config/skills/jz-edit-talking-head-video/config.yml` 的 `draft_folder`；没有则询问并写入配置。一般形如 `.../JianyingPro Drafts`）
