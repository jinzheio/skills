...
```text
~/.config/skills/jz-send-notification/.env
```

也可用 `JZ_NOTIFY_ENV` 指向其它配置文件。脚本读取优先级：

1. `JZ_NOTIFY_ENV` 指向的文件。
2. `~/.config/skills/jz-send-notification/.env`。
3. 当前 shell 已导出的环境变量。
...
如果业务项目必须自带脚本，可复制 `scripts/notify.sh`，但仍使用 `~/.config/skills/jz-send-notification/.env` 或 `JZ_NOTIFY_ENV` 管理配置。
