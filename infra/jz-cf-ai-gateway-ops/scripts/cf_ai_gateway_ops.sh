...
elif [[ -f "${HOME}/.config/skills/jz-cf-ai-gateway-ops/.env" ]]; then
  # shellcheck disable=SC1090
  source "${HOME}/.config/skills/jz-cf-ai-gateway-ops/.env"
...
need() {
  ...
  local value="$2"
  if [[ -z "${value}" ]]; then
    echo "Missing ${name}. Set it in JZ_CF_AI_GATEWAY_OPS_ENV, ~/.config/skills/jz-cf-ai-gateway-ops/.env, .dev.vars, or the shell." >&2
    exit 2
  fi
}
