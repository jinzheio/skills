...
version: "1.0.0"
description: "为网站或 Web 应用实现转化漏斗事件追踪。触发语包括 track conversion、加埋点、转化追踪、漏斗事件、event tracking、埋点方案、add analytics events、conversion funnel、track signup、track checkout。不是用于接入 Umami 脚本本身（用 jz-setup-analytics），也不是用于读取已有统计数据（用 site-metrics）。"
---
