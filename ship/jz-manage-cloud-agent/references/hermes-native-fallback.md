...
timeout 180 sudo -u {{ config.server.service_user }} env \
  HOME={{ config.server.install_dir }} \
  HERMES_HOME={{ config.hermes.home_dir }} \
...
```bash
chown {{ config.server.service_user }}:{{ config.server.service_group }} {{ config.hermes.home_dir }}/config.yaml {{ config.hermes.home_dir }}/.env
...
- `.env` 和 `config.yaml` 的 owner/group 为 `{{ config.server.service_user }}:{{ config.server.service_group }}`，mode 为 `600`。
