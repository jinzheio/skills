...
| 当前状态 | 下一步 |
| --- | --- |
| 本地项目还没有 Cloudflare 可验证部署 | `jz-create-cf-site` |
| 已有 Cloudflare 部署，但没有 GitHub repo | `jz-create-cf-site` 的 GitHub 分支，或按用户要求先只部署 |
| 已有 Cloudflare 部署和 GitHub repo，但没有生产自动部署 workflow | `jz-push-code`，由它检查并补 Cloudflare 自动部署 |
| 已有 Cloudflare 临时 URL，用户提供正式域名 | `jz-launch-domain` |
| 正式域名还不可访问 | 不运行 `jz-setup-analytics`，先完成 `jz-launch-domain` |
