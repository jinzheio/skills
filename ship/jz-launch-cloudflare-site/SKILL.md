---
name: jz-launch-cloudflare-site
description: "当用户想把一个新网站或 Web app 从本地代码推进到 Cloudflare 上线，并顺手配置 GitHub、自动部署、正式域名、搜索/统计、转化埋点和 Auto PR 时使用。适用于 deploy this site to Cloudflare、ship this site to Cloudflare、从代码到上线、新站点发布全流程、上线后接 analytics 和 Auto PR。默认新站点发布到 Cloudflare，不处理 Vercel 迁移；迁移旧项目仍单独使用 jz-migrate-vercel-to-cloudflare。"
---
...
```text
jz-deploy-cloudflare -> jz-setup-site-domain -> jz-push-code -> jz-setup-site-analytics -> jz-setup-conversion-tracking -> jz-setup-auto-pr
```

其中 `jz-setup-conversion-tracking` 和 `jz-setup-auto-pr` 是按需阶段。用户没有要求转化埋点或 Auto PR 时，不默认实现。
...
- 从 Vercel 迁移生产流量。使用 `jz-migrate-vercel-to-cloudflare`。
- 只改 DNS。使用 `jz-setup-site-domain`。
- 只推送已有变更。使用 `jz-push-code`。
- 只接统计和搜索。使用 `jz-setup-site-analytics`。
...
- 只有本地代码：先 `jz-deploy-cloudflare`。
- 已有 Cloudflare 部署但没有正式域名：先 `jz-setup-site-domain`。
- 已有正式域名但没统计：运行 `jz-setup-site-analytics`。
...
1. **Cloudflare deploy**：使用 `jz-deploy-cloudflare`。
2. **Domain**：用户给出正式域名时使用 `jz-setup-site-domain`。
3. **Production auto deploy / push**：使用 `jz-push-code`。
4. **Analytics and search**：正式域名可访问后使用 `jz-setup-site-analytics`。
5. **Conversion tracking**：用户要求转化漏斗或已有明确转化路径时使用 `jz-setup-conversion-tracking`。
...
- Cloudflare 发布：`jz-deploy-cloudflare`
- Cloudflare token：`jz-create-cloudflare-token`
- 域名绑定：`jz-setup-site-domain`
- 推送与 Cloudflare 自动部署：`jz-push-code`
- 搜索与统计：`jz-setup-site-analytics`
- IndexNow：`jz-setup-indexnow`
- 转化埋点：`jz-setup-conversion-tracking`
- Auto PR：`jz-setup-auto-pr`
- 旧项目迁移：`jz-migrate-vercel-to-cloudflare`，只在用户明确要求迁移时使用
