---
name: jz-cloud-agent
description: 诊断和运维云端 agent 部署，以及将本地 skill 同步到云端 agent（ProdaOne Hermes/OpenClaw）。登录服务器检查 OpenClaw 和 Hermes 运行状态，排查 Telegram 消息异常、cron job 故障、Codex OAuth 过期、home channel 丢失、support 邮件接收，并通过 SSH 隧道打开远程 Chrome 桌面。触发词包括：agent 不工作、agent 没回复、hermes agent、codex 用量、fallback、cron job 失败、Codex OAuth、home channel、gateway、打开远程桌面、打开 agent 远程桌面、远程 Chrome、noVNC、support 邮箱、客户邮件、clawsimplesupport 群、同步 skill 到 cloud agent、sync skill to cloud agent、把 skill 同步到 ProdaOne、部署 skill 到云端 agent。当问题涉及 Hermes agent 处理邮件或群内回复时，优先按 Hermes 诊断，不要默认改走 OpenClaw support-inbox cron。
---
...
2. **同步 skill 到云端 agent**：把本地项目里的 skill 目录复制到 ProdaOne Hermes（默认）或 OpenClaw 的 skills 目录，检查并补充所需认证环境变量，然后试跑验证。

默认只做读取和诊断。需要修改服务器配置、重启服务、导入 OAuth、修改 Hermes/OpenClaw 文件时，先说明改动点和验证方式。修改后必须复核 owner/group/mode。

## 配置

本 skill 依赖全局配置文件 `~/.config/skills/jz-cloud-agent.yaml`。首次使用时，将 skill 目录下的 `config.example.yaml` 复制到该路径并填入实际值。

也支持将配置文件放在 skill 本地目录（`config.yaml`），作为未配置全局文件时的回退。优先级：`~/.config/skills/jz-cloud-agent.yaml` > `./config.yaml`（skill 目录）。

**向下兼容**：如果以上两个路径都不存在，回退读取 `~/.config/skills/jz-check-cloud-agent.yaml` 和 `~/.config/skills/cloud-agent-ops.yaml`。
...
def validate_config(cfg: dict):
    ...
    if errors:
        msg = "\n\n".join(errors)
        msg += (
            "\n\nEdit ~/.config/skills/jz-cloud-agent.yaml and replace every"
...
def load_config(deployment_id: str | None = None) -> dict:
    ...
    # Priority: jz-cloud-agent global > jz-cloud-agent local > jz-check-cloud-agent global > cloud-agent-ops global
    global_path = os.path.expanduser("~/.config/skills/jz-cloud-agent.yaml")
    local_path = os.path.join(os.path.dirname(__file__), "config.yaml")
    legacy_global = os.path.expanduser("~/.config/skills/jz-check-cloud-agent.yaml")
    cao_global = os.path.expanduser("~/.config/skills/cloud-agent-ops.yaml")
    ...
    if os.path.exists(global_path):
        ...
        path = local_path
    elif os.path.exists(legacy_global):
        path = legacy_global
    elif os.path.exists(cao_global):
        path = cao_global
...
连接地址和 SSH 参数从 `~/.config/skills/jz-cloud-agent/<deployment>.yaml` 读取，不把实际地址写入 skill。
...
当用户说"同步 skill 到 cloud agent""把 xx skill 部署到 ProdaOne""在 Hermes 上试跑 xx skill"时，执行以下流程。

默认目标：**ProdaOne Hermes**（`{{ config.hermes.agent_display_name }}`）。
...
scp -r -o StrictHostKeyChecking=no -i {{ config.server.ssh_identity_file }} \
  <local-skill-dir> {{ config.server.ssh_user }}@{{ config.server.ip }}:/opt/clawsimple/.hermes/main/skills/<skill-name>
```
...
ssh -o BatchMode=yes -o StrictHostKeyChecking=no \
  -i {{ config.server.ssh_identity_file }} {{ config.server.ssh_user }}@{{ config.server.ip }} \
  'chown -R clawsimple:clawsimple /opt/clawsimple/.hermes/main/skills/<skill-name> && \
   find /opt/clawsimple/.hermes/main/skills/<skill-name> -type f -exec chmod 644 {} \; && \
   find /opt/clawsimple/.hermes/main/skills/<skill-name> -type d -exec chmod 755 {} \;'
```

如果 skill 有可执行脚本：
```bash
ssh ... 'find /opt/clawsimple/.hermes/main/skills/<skill-name>/scripts -type f -name "*.sh" -o -name "*.mjs" | xargs -r chmod +x'
```
...
检查 skill 是否被识别：
```bash
sudo -u clawsimple env ... {{ config.hermes.agent_dir }}/venv/bin/python -m hermes_cli.main skills list
```
...
3. 格式：`KEY="value"`，确保 owner 为 `clawsimple:clawsimple 600`
...
如果是 OpenClaw 目标：
```bash
sudo -u clawsimple env HOME={{ config.server.install_dir }} openclaw agent \
  --agent main \
  --message '<prompt>' \
  --thinking minimal \
  --timeout 120 \
