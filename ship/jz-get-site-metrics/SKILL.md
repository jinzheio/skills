---
name: jz-get-site-metrics
description: "实时查看站点 metrics。覆盖 Google Search Console、Cloudflare Analytics、Umami、Microsoft Clarity、Sentry 五个 provider。未指定域名时自动从项目配置中检测。需要查看站点表现、搜索数据、流量、用户行为、错误追踪时触发。触发词包括 '查一下网站的 metrics'、'查看站点数据'、'最近表现怎么样'、'get analytics'、'get metrics'。默认获取全部 provider。"
---
...
- 各 provider 凭据在 `~/.config/skills/jz-get-site-metrics/.env`（本机）或 skill 根目录 `.env`（fallback）

## 输入

- 可选：`--hostname`，例如 `example.com`。未指定时，自动从当前项目配置文件读取。
- 可选：`--days`（默认 28）
- 可选：`--providers` 逗号分隔，默认 `gsc,cloudflare,umami,clarity,sentry`。可选值：`gsc` | `cloudflare` | `umami` | `clarity` | `sentry`
...
`未检测到站点域名，请提供 --hostname，例如 example.com`

## 流程

1. 解析参数：若 `--hostname` 为空，执行站点自动发现
2. 确认检测到的 hostname，或等待 30 秒超时
3. 加载凭据：先读 `~/.config/skills/jz-get-site-metrics/.env`，缺失的变量 fallback 到 skill 根目录 `.env`
4. **先跑脚本**：`python3 <skill-dir>/scripts/get_analytics.py --hostname <hostname> --days <N> --providers gsc,cloudflare,umami,clarity,sentry`
...
**无论如何不能遗漏任何 provider。** 即使脚本失败，也必须按自然语言逐个 provider 做 fallback 获取。汇报时必须列出全部 5 个 provider，每个标注实际状态。没有例外。
...
```bash
# 1. 从 env 文件读凭据
grep UMAMI ~/.config/skills/jz-get-site-metrics/.env
# 2. 登录获取 token
...
2. 如果提示 token 缺失，检查 `~/.config/skills/jz-get-site-metrics/site-integrations.json` 是否存在并按 hostname 匹配。

**Sentry fallback**：
1. 如果 token 缺失或权限不足，检查 `~/.config/skills/jz-get-site-metrics/.env` 中 `SENTRY_API_TOKEN` 是否存在且具备 `project:read`、`org:read` 权限。
2. 如果 API 返回 404（项目未找到），可能项目尚未通过 `jz-setup-site-analytics` 创建——报告为 `skipped` 并说明原因。

### 汇报模板

最终汇报必须按此格式，5 个 provider 全部列出：
...
- `~/.config/skills/jz-get-site-metrics/site-integrations.json`
...
- 项目 issues 统计（总数、未解决、近期频率）
- project health（firstEvent、lastEvent、event 计数）
- 错误率趋势

凭据：`SENTRY_API_TOKEN`（需 `project:read`、`org:read` 权限）。通过 list projects API 找到 slug 匹配 repo 根目录名的项目，再查询其 issues 和 stats。

## 凭据加载

```bash
# 本机凭据（优先）
~/.config/skills/jz-get-site-metrics/.env

# fallback
<skill-dir>/.env
```

`~/.config/skills/jz-get-site-metrics/.env` 已 `.gitignore`，不会被提交。
...
调用方式：
```bash
python3 <skill-dir>/scripts/get_analytics.py \
  --hostname example.com \
  --days 28 \
  --providers gsc,cloudflare,umami,clarity,sentry
```
...
按 provider 列出。必须覆盖全部 5 个 provider，缺一不可：
