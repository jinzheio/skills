#!/usr/bin/env python3
"""jz-get-site-metrics — 实时查看站点 metrics（GSC / Cloudflare / Umami / Clarity）."""
import argparse
import json
import os
import re
...
USER_CONFIG_ENV = Path.home() / ".config" / "skills" / "jz-get-site-metrics" / ".env"
SKILL_ENV = SKILL_ROOT / ".env"
SITE_INTEGRATIONS_PATH = Path(
    os.environ.get(
        "SITE_INTEGRATIONS_CONFIG",
        str(Path.home() / ".config" / "skills" / "jz-get-site-metrics" / "site-integrations.json"),
    )
).expanduser()
...
def main():
    parser = argparse.ArgumentParser(description="jz-get-site-metrics — 实时查看站点 metrics")
    ...
    if not hostname:
        print(
            json.dumps(
                {"error": "未检测到站点域名，请提供 --hostname，例如 example.com"},
                indent=2,
                ensure_ascii=False,
            ),
            file=sys.stderr,
