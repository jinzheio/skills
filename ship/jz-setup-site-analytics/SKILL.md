...
1. `jz-deploy-vercel` 或 `jz-deploy-cloudflare`
2. `setup-site-domain`
...
如果站点只在平台临时 URL 上，或自定义域名还在传播，停止并先用 `setup-site-domain`。
...
- `GCP_QUOTA_PROJECT` — 已启用 Search Console + Site Verification API 的 GCP 项目。未设置时尝试从当前项目配置检测。

### Cloudflare（DNS TXT 验证记录）

- `CLOUDFLARE_ACCOUNT_ID`
- `CLOUDFLARE_API_TOKEN` — 需 DNS 写权限（与 `jz-create-cloudflare-token` 共享）
...
- `SENTRY_API_TOKEN` — Sentry API token（需 `project:read`、`project:write`、`project:admin` 和 `org:read` 权限；从 [Sentry auth tokens](https://sentry.io/settings/auth-tokens/) 创建）
- `SENTRY_ORG_SLUG`（可选）— 优先通过 list projects 自动检测，也可显式设置
...
参考 `references/indexnow.md`。如果 repo 还没有 IndexNow，可 handoff 到 `setup-indexnow`。
