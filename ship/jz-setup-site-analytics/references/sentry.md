...
1. 从凭据文件读取 `SENTRY_API_TOKEN`（仅在 `~/.config/skills/jz-setup-site-analytics/.env` 或 skill 根目录 `.env`）。
...
Token 创建地址：[Sentry Auth Tokens](https://sentry.io/settings/auth-tokens/)
...
未设置时优先通过 list projects 自动检测；无法检测时再要求用户指定。
