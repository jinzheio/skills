---
name: jz-setup-github-agent-access
...
这不是通用的 GitHub collaborator 管理 skill。普通成员通常需要直接访问 upstream；本 skill 强制 agent 只写自己的 fork，并把 upstream 权限限制为只读。

## 本机账号配置

- 优先读取 `~/.config/skills/jz-setup-github-agent-access/.env`。
- `JZ_GITHUB_AGENT_ACCESS_CONFIG` 可以指定其它本机未跟踪配置；旧变量 `ADD_CLOUD_AGENT_COLLABORATOR_CONFIG` 仅作 fallback。
...
```bash
CONFIG_FILE="${JZ_GITHUB_AGENT_ACCESS_CONFIG:-${ADD_CLOUD_AGENT_COLLABORATOR_CONFIG:-$HOME/.config/skills/jz-setup-github-agent-access/.env}}"

if [ -e "$CONFIG_FILE" ]; then
  test -f "$CONFIG_FILE" || {
    echo "GitHub agent access config is not a readable file" >&2
    exit 1
  }
  set -a
  . "$CONFIG_FILE"
