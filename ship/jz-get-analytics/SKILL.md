...
- 各 provider 凭据在 `~/.config/skills/jz-get-analytics/.env`（本机）或 skill 根目录 `.env`（fallback）
...
3. 加载凭据：先读 `~/.config/skills/jz-get-analytics/.env`，缺失的变量 fallback 到 skill 根目录 `.env`
...
```bash
# 1. 从 env 文件读凭据
grep UMAMI ~/.config/skills/jz-get-analytics/.env
# 2. 登录获取 token
...
2. 如果提示 token 缺失，检查 `~/.config/skills/jz-get-analytics/site-integrations.json` 是否存在并按 hostname 匹配。

**Sentry fallback**：
1. 如果 token 缺失或权限不足，检查 `~/.config/skills/jz-get-analytics/.env` 中 `SENTRY_API_TOKEN` 是否存在且具备 `project:read`、`org:read` 权限。
2. 如果 API 返回 404（项目未找到），可能项目尚未通过 `jz-setup-analytics` 创建——报告为 `skipped` 并说明原因。
...
- `~/.config/skills/jz-get-analytics/site-integrations.json`
...
```bash
# 本机凭据（优先）
~/.config/skills/jz-get-analytics/.env

# fallback
<skill-dir>/.env
```

`~/.config/skills/jz-get-analytics/.env` 已 `.gitignore`，不会被提交。
