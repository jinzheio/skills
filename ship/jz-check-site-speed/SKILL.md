...
加载顺序：
1. `~/.config/skills/jz-check-site-speed/.env`（本 skill 的本机配置）
2. `~/.config/skills/jz-get-site-metrics/.env`（共享站点凭据 fallback）
3. skill 根目录 `.env`（旧安装 fallback）
4. `$CWD/.env`（项目 env）
...
```bash
python3 <skill-dir>/scripts/check_pagespeed.py \
