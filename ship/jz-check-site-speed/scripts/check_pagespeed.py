#!/usr/bin/env python3
"""jz-check-site-speed — PageSpeed Insights + CrUX + Cloudflare Web Vitals."""
import argparse
import json
import os
import sys
...
USER_CONFIG_ENV = Path.home() / ".config" / "skills" / "jz-check-site-speed" / ".env"
SHARED_CONFIG_ENV = Path.home() / ".config" / "skills" / "jz-get-site-metrics" / ".env"
LEGACY_SKILL_ENV = SKILL_ROOT / ".env"
...
def load_env() -> dict:
    merged = read_env_file(LEGACY_SKILL_ENV)
    merged.update(read_env_file(SHARED_CONFIG_ENV))
...
def main():
    parser = argparse.ArgumentParser(description="jz-check-site-speed — 页面速度诊断")
